...
public class Log4jAppender extends AppenderSkeleton {
	...
	public synchronized void append(LoggingEvent event) throws FlumeException {
		...
		/*
		 * if (rpcClient == null) { String errorMsg =
		 * "Cannot Append to Appender! Appender either closed or" +
		 * " not setup correctly!"; LogLog.error(errorMsg); if (unsafeMode) {
		 * return; } throw new FlumeException(errorMsg); }
		 * 
		 * if(!rpcClient.isActive()){ reconnect(); }
		 */
		...
		// Don't check ready if we are not in unsafeMode
		if (unsafeMode && !ready) {
			return;
		}
